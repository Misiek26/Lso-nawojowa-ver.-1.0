...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Marzec_dod']!=0)
                $Suma_dod -= (int)$row['Marzec_dod'];
            if((int)$row['Marzec_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Marzec_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Marzec_dod = $points_dod, Marzec_ujemne = $points_ujemne, Marzec = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
