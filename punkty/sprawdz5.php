...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Maj_dod']!=0)
                $Suma_dod -= (int)$row['Maj_dod'];
            if((int)$row['Maj_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Maj_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Maj_dod = $points_dod, Maj_ujemne = $points_ujemne, Maj = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
