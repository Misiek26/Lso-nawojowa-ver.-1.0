...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Luty_dod']!=0)
                $Suma_dod -= (int)$row['Luty_dod'];
            if((int)$row['Luty_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Luty_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Luty_dod = $points_dod, Luty_ujemne = $points_ujemne, Luty = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
