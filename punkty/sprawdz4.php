...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Kwiecien_dod']!=0)
                $Suma_dod -= (int)$row['Kwiecien_dod'];
            if((int)$row['Kwiecien_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Kwiecien_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Kwiecien_dod = $points_dod, Kwiecien_ujemne = $points_ujemne, Kwiecien = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
