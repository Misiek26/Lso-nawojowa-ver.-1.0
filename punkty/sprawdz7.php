...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Lipiec_dod']!=0)
                $Suma_dod -= (int)$row['Lipiec_dod'];
            if((int)$row['Lipiec_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Lipiec_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Lipiec_dod = $points_dod, Lipiec_ujemne = $points_ujemne, Lipiec = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
