...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Grudzien_dod']!=0)
                $Suma_dod -= (int)$row['Grudzien_dod'];
            if((int)$row['Grudzien_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Grudzien_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Grudzien_dod = $points_dod, Grudzien_ujemne = $points_ujemne, Grudzien = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
