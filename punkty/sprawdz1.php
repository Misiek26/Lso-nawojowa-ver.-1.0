...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Styczen_dod']!=0)
                $Suma_dod -= (int)$row['Styczen_dod'];
            if((int)$row['Styczen_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Styczen_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Styczen_dod = $points_dod, Styczen_ujemne = $points_ujemne, Styczen = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
