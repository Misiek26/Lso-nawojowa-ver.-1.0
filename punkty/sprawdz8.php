...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Sierpien_dod']!=0)
                $Suma_dod -= (int)$row['Sierpien_dod'];
            if((int)$row['Sierpien_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Sierpien_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Sierpien_dod = $points_dod, Sierpien_ujemne = $points_ujemne, Sierpien = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
