...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Wrzesien_dod']!=0)
                $Suma_dod -= (int)$row['Wrzesien_dod'];
            if((int)$row['Wrzesien_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Wrzesien_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Wrzesien_dod = $points_dod, Wrzesien_ujemne = $points_ujemne, Wrzesien = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
