...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Pazdziernik_dod']!=0)
                $Suma_dod -= (int)$row['Pazdziernik_dod'];
            if((int)$row['Pazdziernik_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Pazdziernik_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Pazdziernik_dod = $points_dod, Pazdziernik_ujemne = $points_ujemne, Pazdziernik = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
