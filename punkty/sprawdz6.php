...
<body>
    <?php
        ...
        while($row = mysqli_fetch_array($wynik))
        {
            ...
            if((int)$row['Czerwiec_dod']!=0)
                $Suma_dod -= (int)$row['Czerwiec_dod'];
            if((int)$row['Czerwiec_ujemne']!=0)
                $Suma_ujemne += ((int)$row['Czerwiec_ujemne']) * (-1);
                
            $Suma = (int)$Suma_dod + (int)$Suma_ujemne;

            $ins = mysqli_query($conn,"UPDATE punkty SET Czerwiec_dod = $points_dod, Czerwiec_ujemne = $points_ujemne, Czerwiec = $points_Suma, Suma_dod = $Suma_dod, Suma_ujemne = $Suma_ujemne, Suma = $Suma WHERE id_ministranta = $id;");
        }
